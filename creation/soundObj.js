...
SOUNDOBJ.prototype = {
	...
	exit:function(whereToEnter){
		...
		//TODO 
		//FLESH OUT JUMP TO CHILDREN, 
		// OR TAKE IT OUT ALL TOGETHER
		
		/*else{
			//TODO
			//createLoop functionality
			//console.log('no loop made yet')
			var newID =Math.floor( Math.random() * soundObjArray.length)
			...
			//move camera to the right location
			camera.position.x = soundObjArray[newID].globalPos.x+(Math.random()*.1*soundObjArray[newID].radius) - (.05*soundObjArray[newID].radius)
			camera.position.y = soundObjArray[newID].globalPos.y+(Math.random()*.1*soundObjArray[newID].radius) - (.05*soundObjArray[newID].radius)
			camera.position.z = soundObjArray[newID].globalPos.z+(Math.random()*.1*soundObjArray[newID].radius) - (.05*soundObjArray[newID].radius)
			
			//enter this sound object
			this.enter(soundObjArray[newID])
		}*/
	...
	addBlogInfo:function(){
		...
		function infoNoInfo(){
			if(self.info){
				...
				$("#info.blog").html("No Information for this section yet. If you know how to write and would like to review a song, <a href='https://twitter.com/Cabbibo' target='_blank'>Hit me up!</a>")
			}
		}
